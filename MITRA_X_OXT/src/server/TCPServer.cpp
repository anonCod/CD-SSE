...
TCPServer::TCPServer(char *ip, int port, char *dbPath, char *xsetPath) {
    ...
    if (this->sockfd == -1)
        throw new MITRAXOXTException("Couldn't create new socket");
    ...
    if (bind(this->sockfd, (struct sockaddr *)&this->addrServer, sizeof(this->addrServer)) != 0)
        throw new MITRAXOXTException("Socket bind failed");
    if (listen(this->sockfd, 10) != 0)
        throw new MITRAXOXTException("Listen failed");
    ...
    std::cout << "Initialization of MITRAXOXT server..." << std::endl;
    this->MITRAXOXT = new MITRAXOXTServer(dbPath, xsetPath);
...
void TCPServer::handleInitSearch(client_t *client, std::string msg) { // TODO
    std::string answer = "";
    std::vector<std::string> Fw, TList;
    std::string tmp;

    std::cout << "Starting Search: [" << msg << "]" << std::endl;
    while (msg.find_first_of('|') != std::string::npos) {
        TList.push_back(msg.substr(0, msg.find_first_of('|')));
        msg = msg.substr(msg.find_first_of('|') + 1);
    }
    if (!msg.empty())
        TList.push_back(msg);
    ...
    Fw = this->MITRAXOXT->initSearch(TList);

    if (Fw.empty()) { // keyword not found
...
void TCPServer::handleSkimSearch(client_t *client, std::string msg) {
    ...
    this->buff.insert({client->fd, this->MITRAXOXT->skimSearch(allToken, client->t[client->c])});
    client->c += 1;
}
...
void TCPServer::selectLoop() {
    ...
    while (true) {
        ...
        if (fd == -1)
            throw new MITRAXOXTException("Select Error");
