...
CryptoppTools::CryptoppTools(int nu) {
    try { 
        ...
        if (ifile && ifile.is_open()) {
            ...
            this->hexEncoder.Attach(new CryptoPP::StringSink(this->K));
            this->hexEncoder.Put(this->key, this->key.size());
            this->hexEncoder.MessageEnd();
            std::ofstream ofile(K_FILENAME);
            ofile << nu << "|" << this->K;
            ofile.close();
            this->K = this->decodeHex(this->K);
        }
        ...
        throw new MITRAXOXTException(std::string("Key Generation: ").append(e.what()));
    }
}
...
std::string CryptoppTools::my_xor(std::string a, std::string b) { // a "encrypted" with b
    ...
    if (a.size() == 0) {
        ...
        throw new MITRAXOXTException("Mod 0");
    }
    if (b.size() == 0) {
...
bool CryptoppTools::keyFileExist() {
    ...
    if (ifile) {
        std::cout << "Retrieving key from file (" << K_FILENAME << ")" << std::endl;
        return true;
    } else {
        std::cout << "Key file not found (" << K_FILENAME << "), generating new key" << std::endl;
        return false;
    }
}
