...
CryptoppTools::CryptoppTools(int nu) {
    try { 
        ...
        std::ifstream ifile(KG_FILENAME);
        if (ifile && ifile.is_open()) {
            ...
            this->hexEncoder.Attach(new CryptoPP::StringSink(this->Kg));
            this->hexEncoder.Put(this->key, this->key.size());
            this->hexEncoder.MessageEnd();
            std::ofstream ofile(KG_FILENAME);
            ofile << nu << "|" << this->Kg;
            ofile.close();
            this->Kg = this->decodeHex(this->Kg);
        }
...
std::string CryptoppTools::G(std::string ks, std::string h) {
    std::string cipher;

    this->setKeyEncrypt(ks);
...
std::string CryptoppTools::Gtag(std::string kc1, std::string stc) {
    std::string cipher;

    this->setKeyEncrypt(kc1);
    if (stc.size() % CryptoPP::AES::BLOCKSIZE == 0)
        ...
        CryptoPP::StringSource s(stc, true, new CryptoPP::StreamTransformationFilter(this->encrypt, new CryptoPP::StringSink(cipher), CryptoPP::BlockPaddingSchemeDef::ZEROS_PADDING));

    return cipher;
}

std::string CryptoppTools::GtagInv(std::string kc1, std::string stc) {
    std::string cipher;

    this->setKeyDecrypt(kc1);
    if (stc.size() % CryptoPP::AES::BLOCKSIZE == 0)
        ...
        CryptoPP::StringSource s(stc, true, new CryptoPP::StreamTransformationFilter(this->decrypt, new CryptoPP::StringSink(cipher), CryptoPP::BlockPaddingSchemeDef::ZEROS_PADDING));
    return cipher.substr(0, cipher.find_first_not_of("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=|"));
}
...
std::string CryptoppTools::getKg() {
    return this->Kg;
}
...
std::string CryptoppTools::dec(std::string Ke, std::string e) {
    ...
    return cipher.substr(0, cipher.find_first_not_of("0123456789"));
}
...
bool CryptoppTools::keyFileExist() {
    ...
    if (ifile) {
        std::cout << "Retrieving key from file (" << KG_FILENAME << ")" << std::endl;
        return true;
    } else {
        std::cout << "Key file not found (" << KG_FILENAME << "), generating new key" << std::endl;
        return false;
    }
}
...
void CryptoppTools::setKeyEncrypt(std::string keyValue) {
    if(CryptoPP::AES::DEFAULT_KEYLENGTH < keyValue.size()) // Just a security, should never be called
...
void CryptoppTools::setKeyDecrypt(std::string keyValue) {
    if(CryptoPP::AES::DEFAULT_KEYLENGTH < keyValue.size()) // Just a security, should never be called
        keyValue = keyValue.substr(0, CryptoPP::AES::DEFAULT_KEYLENGTH);
    else if(CryptoPP::AES::DEFAULT_KEYLENGTH > keyValue.size())
        keyValue += std::string(CryptoPP::AES::DEFAULT_KEYLENGTH - keyValue.size(), '*'); // pad
