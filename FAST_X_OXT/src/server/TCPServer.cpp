...
TCPServer::TCPServer(char *ip, int port, char *dbpath, char *xsetPath) {
    int optval = 1;

    memset(&this->addrServer, 0, sizeof(this->addrServer));
    this->sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (this->sockfd == -1)
        throw new FASTXOXTException("Couldn't create new socket");
    ...
    if (bind(this->sockfd, (struct sockaddr *)&this->addrServer, sizeof(this->addrServer)) != 0)
        throw new FASTXOXTException("Socket bind failed");
    if (listen(this->sockfd, 10) != 0)
        throw new FASTXOXTException("Listen failed");
    ...
    std::cout << "Initialization of FASTXOXT server..." << std::endl;
    this->FASTXOXT = new FASTXOXTServer(dbpath, xsetPath);
...
void TCPServer::handleUpdate(int fd, std::string msg) {
    std::string u, e, xtag, y;
    bool op;

    std::cout << "Handling Update:" << std::endl;
    u = msg.substr(0, msg.find('|'));
    ...
    if (this->FASTXOXT->update(u, e, xtag, y, op))
        this->buff.insert({fd, "OK"});
    else
        this->buff.insert({fd, "KO"});
}

void TCPServer::handleInitSearch(client_t*client, std::string msg) {
    std::vector<std::string> result;
    std::string tw, stc;
    int c;

    tw = msg.substr(0, msg.find_first_of('|'));
    stc = msg.substr(msg.find_first_of('|') + 1, msg.find_last_of('|') - (msg.find_first_of('|') + 1));
    c = std::atoi(msg.substr(msg.find_last_of('|') + 1).c_str());

    client->timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    result = this->FASTXOXT->initSearch(tw, stc, c);
    
    if (result.empty()) { // keyword not found
...
void TCPServer::handleSkimSearch(client_t *client, std::string msg) {
    ...
    this->buff.insert({client->fd, this->FASTXOXT->skimSearch(allToken, client->t[client->c])});
    client->c += 1;
}
...
void TCPServer::selectLoop() {
    ...
    while (true) {
        ...
        if (fd == -1)
            throw new FASTXOXTException("Select Error");
        ...
        if (fd != 0) { // = not timeout
            ...
            for (auto client = this->clientList.begin(); client != this->clientList.end(); client++) {
                ...
                if (FD_ISSET(client->fd, &this->FDRead)) {
                    ...
                    if (msg[0] == 'U') { // update
                        this->handleUpdate(client->fd, msg.substr(1)); // remove first (U)
                    } else if (msg[0] == 'S') { // first keyword search (FAST)
                        this->handleInitSearch(&(*client), msg.substr(1));// remove first (S) 
                    } else if (msg[0] == 'X') { // conjuctive search (OXT)
                        this->handleSkimSearch(&(*client), msg.substr(1));
                    } else if (msg == "DISCONNECT") { // disconnection
                        this->disconnect(client->fd);
                        client--;
                    } else { // Unknown / corrupted message
                        std::cout << "Unknown message received from " << client->fd << ": [" << msg << "] - ["  << (int)msg[0] << "]" <<std::endl;
                        this->buff.insert({client->fd, "KO"});
                    }
                }
            }
