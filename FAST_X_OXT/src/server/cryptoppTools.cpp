...
CryptoppTools::CryptoppTools() {
    try {
        ...
        throw new FASTXOXTException(std::string("Key Generation: ").append(e.what()));
    }
}
...
std::string CryptoppTools::pseudorandomFunction(std::string k, std::string v) {
    this->setEncKey(k);
    std::string cipher;

    if (v.size() % CryptoPP::AES::BLOCKSIZE == 0)
        CryptoPP::StringSource s(v, true, new CryptoPP::StreamTransformationFilter(this->encrypt, new CryptoPP::StringSink(cipher), CryptoPP::BlockPaddingSchemeDef::NO_PADDING));
    else
        CryptoPP::StringSource s(v, true, new CryptoPP::StreamTransformationFilter(this->encrypt, new CryptoPP::StringSink(cipher)));
    return cipher;
}
...
std::string CryptoppTools::my_xor(std::string a, std::string b) { // a "encrypted" with b
    ...
    if (a.size() == 0) {
        ...
        throw new FASTXOXTException("Mod 0");
    }
    if (b.size() == 0) {
...
void CryptoppTools::setEncKey(std::string keyValue) {
    ...
    if(CryptoPP::AES::DEFAULT_KEYLENGTH < keyValue.size())
        ...
        keyValue += std::string(CryptoPP::AES::DEFAULT_KEYLENGTH - keyValue.size(), '*'); // pad
    memcpy(key, keyValue.c_str(), CryptoPP::AES::DEFAULT_KEYLENGTH);
    this->encrypt.SetKeyWithIV(key, key.size(), this->iv);
}
